 ...
 * Copyright 2009-2011, Jerry Ablan and Pogostick, LLC.
 *
...
require_once __DIR__ . 'Kisma/exceptions.php';
...
class Kisma implements IGlobalDebuggable, ISettings
{
	 ...
	 *
	 * @param \stdClass|array|null $settings
	 *
	 * @return bool
	 */
	public static function initialize( $settings = array() )
	{
		...
		self::so( self::$_settings, \AppConfig::BasePath, __DIR__ );
	 ...
	 * Examples:
	 *	 Class Name:
	 *		 \Kisma\Components\ComponentEvent becomes "kisma.components.component_event"
	 *
	 * @param string $tag
	 *
	 * @return string
	...
	public static function loadModules( $configuration = array() )
	{
		...
		if ( null !== ( $_libPath = self::getSetting( 'library_path' ) ) )
		{
			\set_include_path( $_includePath . PATH_SEPARATOR . $_libPath . PATH_SEPARATOR );
		}

		$_modules = self::o( $configuration, 'modules' );

		foreach ( $_modules as $_moduleName => $_config )
		{
			self::loadModule( $_moduleName, $_config );
		}
	}
	...
	public static function loadModule( $moduleName, $config = array() )
	{
		...
		{
			Log::error( 'Module "' . $moduleName . '" not found. Check your paths.' );
		}
	}
	...
	public static function initializeDatabase( $which = 'primary' )
	{
		...
		\K::so( $_SESSION, 'db', $_db = \K::createComponent( \AppConfig::CouchDbClass, $_options ) );
	}
