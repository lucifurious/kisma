...
class ErrorHandler
{
		...
		try
		{
			...
			Render::twigView(
				...
				array(
					...
					'navbar'            => array(
						'brand' => 'Kisma v' . Kisma::KISMA_VERSION,
						'items' => array(
							array(
	...
	protected static function _cleanTrace( array &$trace, $skipLines = null, $basePath = null )
	{
		...
		foreach ( $trace as $_index => $_code )
		{
			...
			Option::sins( $trace[$_index], 'file', 'Unspecified' );
			Option::sins( $trace[$_index], 'line', 0 );
			Option::sins( $trace[$_index], 'function', 'Unspecified' );
