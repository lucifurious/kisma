...
class Option
{
	 ...
	 * @param array  $options
	 * @param string $key
	 * @param bool   $returnKey If true, the actual key is returned instead of TRUE
	 *
	 * @return bool|string
	 */
	public static function contains( &$options = array(), $key, $returnKey = false )
	{
		$_key = static::_cleanKey( $key );

		//	Check both the raw and cooked keys
		if ( !( $options instanceof \ArrayAccess ) && !is_object( $options ) )
		{
			return false;
		}

		return $options instanceof \ArrayAccess
			? ( isset( $options[$key] ) ? $key : ( isset( $options[$_key] ) ? ( $returnKey ? $_key : true ) : false ) )
			: ( property_exists(
				$options,
				$key
			) ? $key : ( property_exists( $options, $_key ) ? ( $returnKey ? $_key : true ) : false ) );
	}
	 ...
	 * @return array The changed keys
	 */
	public static function setMany( &$options = array(), $data )
	{
		if ( !( $data instanceof \Traversable ) )
		{
			throw new \InvalidArgumentException( 'Expected array or object. $data type incorrect.' );
		}
		...
		{
			$_results[] = static::set( $options, $_key, $_value );
		}
	...
	public static function get( &$options = array(), $key, $defaultValue = null, $unsetValue = false )
	{
		...
		//	Inflect pain!
		$key = static::_cleanKey( $_originalKey = $key );
	 ...
	 * @return mixed
	 */
	public static function setDeep( &$options = array(), $key, $subKey, $value = null )
	{
		$_deep = static::get( $options, $key, array() );
		static::set( $_deep, $$subKey, $value );

		return static::set( $options, $key, $_deep );
	}
	...
	public static function set( &$options = array(), $key, $value = null )
	{
		...
		if ( is_object( $options ) )
		{
			...
			{
				call_user_func( array( $options, $_setter ), $value );
			}
			else
			{
				if ( !property_exists( $options, $key ) && property_exists( $options, $_cleanKey ) )
				{
					$key = $_cleanKey;
				}
	...
	public static function remove( &$options = array(), $key )
	{
		...
		if ( static::contains( $options, $key ) )
		{
			...
			if ( is_array( $options ) )
			{
				if ( isset( $options[$_cleanedKey] ) && !isset( $options[$key] ) )
				{
					$key = $_cleanedKey;
				}
	 ...
	 *
	 * @param string $key
	 * @param bool   $opposite If true, the key is switched back to it's neutralized or camel-cased format
	 *
	 * @return string
	 */
	protected static function _cleanKey( $key, $opposite = true )
	{
		if ( $key == ( $_cleaned = Inflector::neutralize( $key ) ) )
		{
			if ( false !== $opposite )
			{
				return Inflector::deneutralize( $_cleaned, true );
			}
		}
